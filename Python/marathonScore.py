def marathonTaskScore(marathonLength, maxScore, submissions, successfulSubmissionTime):
    if submissions == 1 and successfulSubmissionTime == 1 :
        return maxScore
    
    penalty = (maxScore/2) * (1/marathonLength)*successfulSubmissionTime  
    penalty += 10*(submissions-1)
    if successfulSubmissionTime < 0:
        return 0        
    return max(maxScore - penalty,maxScore/2)
